...
- IXS is also available on BNB Chain. This app only drafts transactions on Avalanche. For product TVL, use vault_terms: combined TVL sums Avalanche and BNB Chain and is a daily snapshot. Call it a daily snapshot even when its freshness status is LIVE. The connected vault context's totalAssets is Avalanche only. Name the scope and timestamp; never label one chain as the combined total.
